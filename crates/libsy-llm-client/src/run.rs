...
/// Closed VGR decision labels allowed to reach a terminal response.
///
/// These values are generated by VGR from closed enums.  Do not copy an open
/// metadata namespace here: terminal response metadata is persisted by the
/// server's routing log.
const VGR_EVIDENCE_KEYS: [&str; 6] = [
    "switchyard.vgr.predicted",
    "switchyard.vgr.effective",
    "switchyard.vgr.served",
    ...
    -> Result<()> {
        ...
        request.metadata = Some(switchyard_protocol::Metadata {
            extra_metadata: Some(BTreeMap::from([
                ...
                (
                    "switchyard.vgr.unbounded".to_string(),
        ...
        assert!(!extra.contains_key("switchyard.vgr.unbounded"));
