...
//! boxed error — the exact item type carried by
//! [`switchyard_llm_client::RawEventStream`], so the client's streamed wire
//! events frame without an adapter. Contract: OpenAI Chat emits `data: {json}`
//! frames plus a terminal `data: [DONE]`; Anthropic and Responses emit named
