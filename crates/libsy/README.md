...
This complete `src/main.rs` routes a request with `LlmTaskClassifier`. The first
run lets libsy call each target's client. The second drives the step stream so
the host can perform or override each model call itself.
