...
/// Wrap an algorithm so answer requests receive the prompt configured for their target.
///
/// Terminal requests are prepared automatically. An algorithm that produces an answer while
/// routing must make that call with [`Driver::call_answer_model`].
...
mod tests {
    ...
    async fn target_prompts_follow_selected_and_fallback_targets() -> Result<()> {
        ...
        let preserved = strong
            .llm_request
            .preservation
            .requests
            .get(&"openai_chat".into())
            .ok_or_else(|| test_error("missing preserved OpenAI Chat request"))?;
        assert_eq!(preserved["model"], "strong");
        assert_eq!(
            preserved["messages"][0],
            json!({"role": "system", "content": "strong prompt"})
        );
        assert_eq!(
