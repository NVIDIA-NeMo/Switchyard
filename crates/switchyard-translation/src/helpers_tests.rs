...
fn aggregated_response_round_trips_and_stamps_the_served_model() -> Result<(), BoxError> {
    ...
    let encoded = encode_aggregated_response(&agg, WireFormat::OpenAiChat, Some("served/model"))?;
...
fn encode_stream_reassembles_deltas_and_finishes() -> Result<(), BoxError> {
    ...
    let events = block_on(
        encode_stream(chunks, WireFormat::OpenAiChat, Some("m".to_string()))?.collect::<Vec<_>>(),
    )
    .into_iter()
...
fn encode_stream_falls_back_to_the_source_model() -> Result<(), BoxError> {
    ...
    let events =
        block_on(encode_stream(chunks, WireFormat::AnthropicMessages, None)?.collect::<Vec<_>>())
            .into_iter()
            .collect::<Result<Vec<Value>, BoxError>>()?;
...
fn encode_stream_keeps_trailing_usage_after_a_normal_stop() -> Result<(), BoxError> {
    ...
    .boxed();
    let events = block_on(encode_stream(chunks, WireFormat::OpenAiChat, None)?.collect::<Vec<_>>())
        .into_iter()
        .collect::<Result<Vec<Value>, BoxError>>()?;
    let body = serde_json::to_string(&events)?;
    assert!(
