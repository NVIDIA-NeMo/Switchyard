...
| Libsy metrics in `switchyard-server` | Record request-flow metrics through libsy's OpenTelemetry instruments. The Rust server owns the process-wide Prometheus exporter and exposes it at `GET /metrics`; do not add a second accumulator. |
