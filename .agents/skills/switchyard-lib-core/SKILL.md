...
| Routing audit events | Call `record_routing_event(ctx, name, payload)` with an algorithm-owned JSON payload. The framework adds request ID, sequence, and timestamp and optionally writes JSONL. Traces are append-only observability output and must never choose a route. Gate raw content with `routing_trace_content_enabled()`. |
...
| Making the trace framework understand algorithm-specific fields. | Keep typed decisions in the owning profile and copy whatever evidence is useful into the event's opaque payload. |
