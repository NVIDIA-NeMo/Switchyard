...
| Stats / telemetry | Metrics are OpenTelemetry. The chain executor (`profiles/chain.py`) records request/latency/token/cost/TTFT/routing metrics via `switchyard.lib.metrics` record helpers; `switchyard.lib.otel_usage` extracts usage from responses. Do not write a parallel collector or reintroduce a `StatsAccumulator` stack. |
...
| Writing a new stats collector or reintroducing a `StatsAccumulator` stack. | Metrics are OpenTelemetry: record via `switchyard.lib.metrics` helpers from the chain executor / `otel_usage`. `/metrics` is served from the OTel Prometheus registry by `endpoints/metrics_endpoint.py`. |
