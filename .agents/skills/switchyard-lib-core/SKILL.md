...
| Error-rate / retry-recovery counters | Use `switchyard.lib.endpoints.outcome_metrics` for logical client outcomes. A retrying Python backend records each upstream attempt itself (and `record_retry_recovered()`) and sets `CTX_UPSTREAM_ATTEMPTS_RECORDED` so the endpoint skips its fallback — `LatencyServiceLLMBackend.call` is the reference. Rust native OpenAI Chat may replay one positively proven null-SSE EOF inside its lazy stream; that remains one logical endpoint outcome, while `StatsAccumulator.openai_transport` separately records physical attempts, null-EOF retries, unpriced retries, and a retry-token sensitivity estimate. Other Rust native / passthrough / multi calls rely on the endpoint fallback (`record_upstream_attempt_success` / `record_upstream_attempt_failure` in `upstream_error.py`, called from `dispatch_chat_request` and `handle_chain_exception`) for the logical attempt. Don't add a `model` label — these counters are layer-aggregate. Keep labels bounded. |
