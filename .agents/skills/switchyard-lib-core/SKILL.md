...
| Stats / telemetry | Metrics are OpenTelemetry on both paths. The Python chain executor (`profiles/chain.py`) records request/latency/token/cost/TTFT/routing metrics via `switchyard.lib.metrics` record helpers (`switchyard.lib.otel_usage` extracts usage); the Rust v2 profiles record via `switchyard_components::otel_metrics`. Do not write a parallel collector or reintroduce a `StatsAccumulator` stack in either language. |
...
| Per-endpoint attribution for a Python backend (e.g. latency-service) | Set `ctx.selected_model = endpoint_id` before returning the response. Also set `ctx.backend_call_latency_ms = upstream_call_ms` so the chain executor records upstream-only latency. `LatencyServiceLLMBackend.call` is the reference. |
| State metrics on `/metrics` | Register a snapshot callable via `switchyard.lib.metrics.register_latency_service_source(...)` (clear with `None` on `shutdown()`); observable gauges/counters in `metrics.py` read it on each scrape. This is for backend-owned state, not request-flow counters. `LatencyServiceLLMBackend._metrics_snapshot` is the reference. |
| Error-rate / retry-recovery counters | Use `switchyard.lib.metrics` (`record_client_response`, `record_upstream_attempt`, `record_retry_recovered`; classify status via `metrics.classify`/`metrics.code_label`). FastAPI middleware records client outcomes. A retrying Python backend records each upstream attempt itself (and `record_retry_recovered()`) and sets `CTX_UPSTREAM_ATTEMPTS_RECORDED` so the endpoint skips its fallback — `LatencyServiceLLMBackend.call` is the reference. Single-attempt backends (Rust native / passthrough / multi) record nothing themselves; the endpoint fallback (`record_upstream_attempt_success` / `record_upstream_attempt_failure` in `upstream_error.py`, called from `dispatch_chat_request` and `handle_chain_exception`) counts their one attempt. Don't add a `model` label — these counters are layer-aggregate. Keep labels bounded. |
...
| Writing a new stats collector or reintroducing a `StatsAccumulator` stack (Python or Rust). | Metrics are OpenTelemetry: record via `switchyard.lib.metrics` helpers (Python chain executor / `otel_usage`) or `switchyard_components::otel_metrics` (Rust v2 profiles). `/metrics` is served from the OTel Prometheus registry by `endpoints/metrics_endpoint.py` (Python app) and the Rust `switchyard-server`. |
