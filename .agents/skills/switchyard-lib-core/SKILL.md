...
| Route sub-agent traffic to a fixed worker target | Add the `SubagentOverride` classifier (`crates/libsy/src/algorithms/util/subagent.rs`) to a `FallThrough` cascade: it scores one fixed worker target for delegated work and abstains otherwise, so later classifiers serve ordinary traffic. Detection is `Metadata::from_headers` + `Metadata::is_subagent_work` in `crates/protocol/src/metadata.rs` — the canonical lineage fact and work-vs-maintenance policy. Do not re-implement header sniffing inside individual profiles or algorithms. Rust-only today; there is no profile-envelope key and no Python binding for it. |
| Keep a sub-agent on whatever target it was first routed to | Register `AffinityRouter` (`crates/libsy/src/algorithms/util/affinity.rs`) with `FallThrough::with_component`, which wires its `Processor` and `Classifier` roles from one `Arc` — registering the two separately is easy to half-wire, and a missing processor leaves the classifier permanently empty. Order it before the classifier that seeds the decision. Opt-in; nothing enables it by default. Distinct from the `session_affinity` / Redis pin store in the row below, which is the `switchyard-core` session cache, not a libsy classifier. |
