...
| Routing audit events | Call `switchyard.lib.routing_trace.record_routing_event(...)` to append normalized `RoutingEvent` values and honor process-level JSONL capture. Keep each profile's typed decision as the source of routing behavior; traces are append-only observability output and must never be read to choose a route. Gate raw content with `routing_trace_content_enabled()`. |
...
| Replacing profile-specific decision types with a generic routing-trace payload. | Keep typed decisions in the owning profile and project them into append-only `RoutingEvent` audit records after each decision is made. |
