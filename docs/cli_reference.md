...
| `--routing-log-file PATH` | Append one JSONL routing record per request (task, session, actual model, tier, token and cache usage) to PATH. Supported by route-bundle and v2 profile-config serve. Also exposes `GET /v1/routing/session-stats?session_id=...`. |
