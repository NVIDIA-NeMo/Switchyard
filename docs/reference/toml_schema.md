...
| `reasoning_format` | No | unset | OpenAI Chat field the target expects assistant reasoning under: `"openai"` for `reasoning`, `"deepseek"` for `reasoning_content`. After encoding, assistant reasoning is moved to this field and the other spelling is removed; the two are never sent together. Unset keeps the encoded field unchanged. Rejected on non-`openai_chat` clients. Use it when a model only accepts reasoning replayed under the exact field it produced. Targets with different format settings need distinct model IDs, same as `reasoning_effort` and `extra_body`. |
